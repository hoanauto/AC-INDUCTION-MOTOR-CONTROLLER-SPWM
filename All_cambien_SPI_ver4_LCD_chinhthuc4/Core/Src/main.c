...
int main(void)
{
  ...
  HAL_Delay(4000);  // thong diep 4s
  CLCD_I2C_Clear(&LCD1); // Xóa thong diep
  /* USER CODE END 2 */
  ...
  {		
	...
	while (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_3)== GPIO_PIN_SET)
		...
		// kiem tra trang thai
	tinhieului = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_4);
		if ( tocdo ==0 && tinhieului == 1)
		{ state_xe = 0;}
		else if (tocdo ==0 && tinhieului == 0)
		{ state_xe = 1;}
	...
	if (state_xe ==1)
	{
		...
		else if(level_tayga !=0)
		{
			if (level_tayga > set_leveltayga)
				...
				{ new_leveltayga =0;
					if((float)tocdo < (float)0.75*want_tocdostator)
